...
router.get('/food_list', function (req, res, next) {
  try {
    ...
    if (!admin) { res.render('loginpage', { message: '' }) }
      ...
      try {
        pool.query("select F.*,(select C.categoryname from category C where C.categoryid=F.categoryid) as categoryname,(select S.subcategoryname from subcategory S where S.subcategoryid=F.subcategoryid) as subcategoryname from foods F", function (error, result) {
          if (error) {
...
router.get('/display_by_id', function (req, res, next) {
  ...
  try {
    pool.query("select F.*,(select C.categoryname from category C where C.categoryid=F.categoryid) as categoryname,(select S.subcategoryname from subcategory S where S.subcategoryid=F.subcategoryid) as subcategoryname from foods F where foodid=?", [req.query.foodid], function (error, result) {
      if (error) {
        res.render('displaybyid', { data: [], message: "Error:Pls Contact to Database Administrator...", status: false })
      }
      else {

        res.render('displaybyid', { data: result[0], message: "Success", status: true })
      }
